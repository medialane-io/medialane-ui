...
import { hypothesisLine } from '../../.ds-sync/lib/common.mjs';
...
try {
  ...
  try {
    ...
    const { renderHashFor } = await import(new URL('../../.ds-sync/lib/sync-hashes.mjs', import.meta.url).href);
    const stale = [];
    if (manifest) {
...
if (!pw) {
  ...
  const { serveDir } = await import(new URL('../../.ds-sync/storybook/http-serve.mjs', import.meta.url).href);
  ...
  try {
    ...
    for (const rel of sample) {
      ...
      try {
        ...
        // Let CSS/JS mount transitions (framer-motion entrance fades, etc.)
        // finish before the screenshot, then force a real paint: after a
        // same-page navigation, headless Chromium can screenshot before the
        // new document has actually painted even though computed styles
        // already read correctly (a double rAF round-trip fixes this).
