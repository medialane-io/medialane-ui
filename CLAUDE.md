...
(Remix/Transfer/Cancel/View owner/Report), price chip, indexing badge, all
preserved. Hrefs are computed internally via `@medialane/sdk`'s `assetHref`/
`collectionHref` using `token.chain` — genuinely chain-aware, not hardcoded to
Starknet. `OfferDialog`/`ReportDialog` stay app-local (wallet/auth-coupled);
the component takes `onOffer`/`onReport` callbacks instead of rendering its own
dialogs — omit either to hide that action, matching the `onList`/`onTransfer`/
`onCancel`/`onBuy` convention this component already had for the write
actions. `RarityTier` and the unused `showBuyButton`/`rarityTier`/`inCart`/
`onCart`/`overflowMenu`/`className` props are removed — zero consumers in
either app. New dependency: `@radix-ui/react-dropdown-menu` (peer + dev) — the
package's first Radix primitive; ported as `src/components/dropdown-menu.tsx`,
the same shadcn output both apps already carry locally, now also exported from
the package root for any future consumer. SDK peer floor raised to `>=0.72.0`
(needs the `routes` module). Both apps must replace their local
`token-card.tsx` with a thin wrapper — see
`medialane-core/docs/plans/2026-07-20-token-card-promotion.md`.) Prior 0.74.2:
(raised the `@medialane/sdk` peer floor `>=0.6.0` → `>=0.71.0`. The loose range let a consumer resolve a stale SDK missing exports this package uses (`hasCapability`, `Api*` types) — bun pulled `0.9.2` during a dao install; the floor also guarantees the clean-root SDK (0.71.0+, no `./starknet` re-export), so ui's chain-neutral imports never drag starknet.js into a consumer's bundle. Dev dep aligned to 0.71.1; **no component code changed** — dist is byte-equivalent to 0.74.1 apart from the version, so consumers on 0.74.1 need not rush the bump.) Prior 0.74.1: (Creator's Fund family — homepage `CreatorAirdropBanner` lifted into the package (prop: `href`; both apps' `creator-airdrop.tsx` are re-export shims) and `CommunityRewardsSection` redesigned to match: shared uppercase `CREATOR'S FUND` eyebrow in `brand-orange`, borderless `bg-muted/50` light / `bg-card` dark panels, accent only as eyebrow + `brand-orange/20` hairlines + solid-orange CTA — no gradient frames/blobs/animated borders/gradient CTAs anywhere in the pair. Info tiles are a hairline-divided row; member cards flat with a 2px solid orange top rule. Spec: `medialane-core/docs/specs/2026-07-17-creators-fund-family-redesign.md`. Note: **0.74.0 is deprecated on npm** — published from a checkout missing 0.73.5–0.73.6; 0.74.1 is the good release.) Prior 0.73.6: (`AssetMarketplacePanel`'s owner listing-cancel button label changed from "Cancel" → "Cancel Listing" — clearer next to the "Your active offer" row's own "Cancel", user feedback.) Prior 0.73.5: `MedialaneCollectionCard` now follows the app's light/dark theme instead of being permanently dark — light is the CSS default (no `.dark` ancestor needed), `.dark` overrides restore the original foil-on-navy look. `.ml-card-material`/`.ml-card-sheen` in `medialane.css` carry both variants; the sheen swaps `mix-blend-mode` from `screen` (white highlight, dark) to `multiply` (soft brand-tint, light) — a white "screen" bloom reads as premium on navy but washes out invisibly on a near-white card, so it isn't just a color swap.) Prior 0.73.4: NavBrandButton left padding 6→10px — the pill reads symmetric (round icon left, menu lines right). 0.73.3: launchpad cards: resting hairline in each group's accent hue (was hover-only); on hover-capable devices the full-spectrum gradient ring fades in over it (.ml-grad-hover-border — decoration only, mobile loses nothing). 0.73.2: launchpad copy: Single Editions card → "Single Edition NFTs"; the "NFTs" group pill → "Originals" (nearly every service mints NFTs, so it wasn't a differentiator). Prior 0.73.1: IP Club launchpad card copy rewritten to the membership-tiers model — no entry-fee/member-cap/open-close vocabulary; tiers, validity window, trade like any collection. Prior 0.73.0: `AssetPicker` + `LicenseTermsBuilder` — the two shared components the
